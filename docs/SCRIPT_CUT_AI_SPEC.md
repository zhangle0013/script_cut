...
**约定**：JSON 管 **时间线与枚举字段**；`description` 管 **Seedance 可执行的画面提示**（构图/光影/空间/氛围为主，**不含**表演动词链，见 §5.7）。`trk_action` 只写 **节拍摘要**，**禁止**复述 `description`（§5.9）。生成顺序：**先 Timing Spec 排口播、镜长 → 为确有节拍处写短 `action` → 再润色 `description`**；勿用动作轨复制画面长文。
...
- 凡剧本/分镜里出现 **可被表演的动词链**（走、停、递、看、转身、点头等），优先在 **`trk_action`** 上落 **带时间的** `clips`，不要把它们只藏在 `description` 长句里；**但** `action` 的 `text` 必须是 **节拍摘要**，**禁止**与同镜 `description` 全文复述（§5.9）。
- 若一镜内有多拍动作，按时间顺序多条 `action` clip，`start/end` **可重叠对白**，与同镜 `visualSegments` 时间范围一致或为其子区间；每条 `text` 仍须 **短**、**不复制**画面段 prose。
...
| 同一时间段内，**画面 `description`** 与 **`trk_action.text` 全文或长片段相同**（常见 AI 劣质输出） | 双轨 **信息冗余**，违背 §5.9 | **动作轨只保留节拍摘要**（≤40 字级）；画面保留完整镜头 prose。**禁止**把 `description` 粘贴到 `trk_action` |
...
### 5.9 `trk_action` 的 `text`：节拍摘要，**禁止复述画面**（硬性）

与同时间段 **`visualSegments[].description` 相比，`clips[]`（`trackId → trk_action`）的 `text` 必须是另一类信息**：只回答「**谁在做什么调度 / 节拍**」，**不得**第二次描写镜头里看见了什么（构图、光影、锈蚀质感、屏幕 UI 文案长描述等一律留在画面段）。

| 动作轨 `text` **应该**写 | **禁止** |
|--------------------------|----------|
| **精炼**走位 / 手势 / 递接 / 对峙节拍（动词开头或「节拍：…」式短句均可） | 把某段 `description` **原样复制**或 **同义改写整段** 再贴到 `trk_action` |
| 一条动作块对应 **一个可表演的节拍**；多条则按时间拆开 | 为「铺满时间」而把画面长文拆成多条动作 —— **仍算复述** |
| 纯固定机位、仅表情或仅 HUD/界面叙事、**没有新增肢体调度** | **宁可不写**该时段的 `action` clip，也不要复制画面文案占轨 |

**自检（生成前强制）**：

- 若某条 `trk_action.text` 与 **时间重叠**的任一 `visualSegments[].description` **全文相同**，或 **连续 20 字以上相同且语序基本一致** → **不合格**：删除该条 **或** 改写为 **≤40 字**的节拍摘要（只保留主体 + 动作结果，删掉一切景别/光影/屏幕细节描写）。
- **禁止**用动作轨承载「导演陈述镜头」——那是 `description` 的职责。

**示例**（画面段已写「一辆严重锈蚀的电动车急刹停在前景。沙尘扑过来，车身铁皮大面积剥落，裂屏上亮着暗蓝色待机光。」）：

- ✅ 动作轨：`急刹停稳；沙尘扑脸` 或 `节拍：车怼到前景`
- ❌ 动作轨：再把「锈蚀、剥落、裂屏、暗蓝光」整段写一遍（与画面 **高度雷同**，禁止）

**与 §7.2 的关系**：动作块时长仍按强度档估 `start/end`；**不是**每个画面段都必须配一条「等长」动作块。**条数 ≤ 独立节拍数**；若无独立节拍则允许动作轨在该区间为空。
...
- **`trackId → trk_action`**：`text` 遵守 **§5.9**（节拍摘要，禁止复述同时间段画面 `description`）
...
### 7.2 动作 / 表演（`trk_action`：`text` 遵守 §5.9；**勿为填满轨而复述画面**）

- **有**独立走位、递接、肢体语言或对峙节拍时：写在 **`trk_action`** 的 `clips` 里，且 **`text` 精炼**，不要把 `description` 整段再写一遍（§5.9）。
- **无**独立节拍（纯表情特写、纯界面/HUD、纯气氛固定镜）：该时段 **允许没有** `action` clip——**禁止**复制画面文案冒充动作轨。
- 不按字数估时长；用 **强度档位** 默认时长（`meta` 中标注 `intensity:S` 等便于自检）：
...
AI 应为每条动作块标 `intensity`，并取区间中值作为默认时长。
...
- [ ] **动作已拆轨**：确有走位/节拍时在 `trk_action` 有落点；**未**把大量动词链只堆在 `visualSegments[].description`（见 §5.7）；且 **未**为用满轨而把画面全文贴进动作轨（§5.9）
- [ ] **动作≠画面复读**：每条 `trk_action.text` 与重叠时段的 `visualSegments[].description` **非全文相同**、无 **≥20 字连续雷同**；否则删条或改为 ≤40 字的节拍摘要（§5.9）
...
| **动作轨** | §7.2 按强度档 S/M/L/XL 区间；`text` 勿复述画面（§5.9） | ❌ 按档位，不按字数；**勿**把 `description` 当动作轨正文 |
